...
<span class="old-price"><del>${L(T)}</del></span>`}else C=T>0?`<span class="new-price item_price">${L(T)}</span>`:`<span class="new-price item_price">Li\xean hệ</span>`;let w="https://bit.ly/hGWr7r",A=p.match(/<img[^>]+src="([^">]+)"/);A&&A[1]&&(w=A[1].replace(/\/s[0-9]+(-c)?\//,"/w400-h225-c/")),i+=`
            ...
            </div>`}i+="</ul>",t.innerHTML=i}function renderProductItem(e){let t=e&&e.title&&e.title.$t?e.title.$t:"Ti\xeau đề kh\xf4ng c\xf3",i="";if(e.link){for(let n=0;n<e.link.length;n++)if("alternate"===e.link[n].rel){i=e.link[n].href;break}}let a="content"in e?e.content.$t:"summary"in e?e.summary.$t:"",v="https://blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEhd6hRb8uI-t1hrf5z1RxcAD037Gr-8GT3l6i4r_2FJYIxypTlViNDf8cxwzXxUfAIyjJ2upQT3HykeFspb5wYwS7COmddSv18Dn9n0HKQvxEK80FaxbeWqoKUybE7AERbbCW7XdnFyd4ZB7TJ6xClLLsAI70DX3IFZK6xOVXe2eXhFfsiNoyN-CcJmz8Q/s320/logo%20DTC%20PNG-small.png",_=a.match(/<img[^>]+src="([^">]+)"/);_&&_[1]&&(v=_[1].replace(/\/s[0-9]+(-c)?\//,"/w400-h225-c/"));let h="",s="",f=e=>isNaN(e)?"Li\xean hệ":e.toLocaleString("vi-VN")+"₫",productIdMatch=a.match(/\[ProductID\](.*?)\[\/ProductID\]/);if(productIdMatch&&productDatabase){let productId=productIdMatch[1],productData=productDatabase[productId];if(productData){let basePriceNum=parseFloat(String(productData.BasePrice).replace(/,/g,""));if(productData.isPromoActive){let promoPriceNum=parseFloat(String(productData.PromoPrice).replace(/,/g,"")),percent=0;basePriceNum>0&&(percent=Math.round((basePriceNum-promoPriceNum)/basePriceNum*100)),percent>0&&(s+=`<span class="product-status discount">Giảm ${percent}%</span>`),h=`<span class="new-price item_price">${f(promoPriceNum)}</span>
<span class="old-price"><del>${f(basePriceNum)}</del></span>`}else h=`<span class="new-price item_price">${f(basePriceNum)}</span>`}else h=`<span class="new-price item_price">Li\xean hệ</span>`}else h=`<span class="new-price item_price">Li\xean hệ</span>`;let r=e=>{let t=`[${e}]`,i=`[/${e}]`;return a.includes(t)&&a.includes(i)?a.substring(a.indexOf(t)+t.length,a.indexOf(i)):""},u=r("tinhtrang");u&&(s+=`<span class="product-status tinhtrang">${u}</span>`);let p=r("daban");return p&&(s+=`<span class="product-status daban">Đã bán: ${tinhngay(p)}</span>`),`
      <li>
