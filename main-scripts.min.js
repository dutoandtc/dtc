...
// 1. Hàm hiển thị lỗi (Chỉ hiện khi có sự cố thực sự)
function showDtcError(msg) {
    let errBox = document.getElementById("dtc-debug");
    if (!errBox) {
        errBox = document.createElement("div");
        errBox.id = "dtc-debug";
        errBox.style = "position:fixed;top:0;left:0;z-index:9999;background:red;color:white;padding:5px;font-size:10px;";
        document.body.appendChild(errBox);
    }
    errBox.innerHTML += "<div>" + msg + "</div>";
}
// 2. Hàm chạy đồng hồ và giảm suất (Logic mới nhất)
function flashsalerun(dateStr, initialSlots) {
    try {
        let p = dateStr.split("/");
        let endTime = new Date(p[2], p[1] - 1, p[0]).getTime();
        ...
        let timer = setInterval(function() {
            ...
            // Cập nhật đồng hồ (Ngày/Giờ/Phút/Giây)
            ...
            // LOGIC GIẢM SUẤT: Giảm dần theo thời gian, tối thiểu còn 2
            // Cứ mỗi 10 phút (600.000ms) sẽ giảm 1 suất để tạo hiệu ứng khách mua liên tục
            let drop = Math.floor(now / 600000) % initialSlots;
            let current = initialSlots - (drop % (initialSlots - 1));
            if (current < 2) current = 2;

            let slotEl = document.getElementById("slot-count");
            if (slotEl) slotEl.innerHTML = current;

        }, 1000);
    } catch (e) { showDtcError("Lỗi Timer: " + e.message); }
}
...
// 3. Hàm chi tiết sản phẩm (Xây dựng lại hoàn toàn để chèn đúng vị trí)
function chitietsp(id, title, i) {
    try {
        window.stopgiamgia = 1;
        let el = document.getElementById(id);
        if (!el) return;

        // Công cụ lấy dữ liệu từ thẻ [ ] cực mạnh (Regex)
        let getVal = (tag, def) => {
            let reg = new RegExp("\\[" + tag + "\\]([\\s\\S]*?)\\[\\/" + tag + "\\]", "i");
            let m = el.innerHTML.match(reg);
            return (m && m[1]) ? m[1].replace(/<[^>]*>?/gm, '').trim() : def;
        };

        // Bóc tách dữ liệu
        let giaBan = getVal("giaban", "Liên hệ");
        let giaCu = getVal("giacu", "");
        let soSuatHienTai = parseInt(getVal("sosuat", "10")) || 10;
        let ngaySale = getVal("flashsale", "");
        let moTasp = getVal("chitiet", "Đang cập nhật...");
        let tinhTrangsp = getVal("tinhtrang", "Còn hàng");
        let daBanNgay = getVal("daban", "");

        // Xử lý ảnh và ngày đã bán
        let imgs = el.getElementsByTagName("img");
        let anhChinh = imgs.length > 0 ? imgs[0].src : "";
        let txtDaBan = daBanNgay ? "<p style='color:#777;'>✔️ Đã bán: " + tinhngay(daBanNgay) + " suất</p>" : "";

        // Xử lý mã giảm giá (Biến d)
        let o = {}, d = "", giamGiaRaw = getVal("giamgia", "");
        if (giamGiaRaw) {
            giamGiaRaw.split(",").forEach(v => {
                let parts = v.split("--");
                if (parts.length == 2) o[parts[0].trim()] = parts[1].trim();
            });
            let k = displayRandomNumber();
            let keys = Object.keys(o);
            let key = keys[k % keys.length];
            let giaGocNum = parseInt(giaBan.replace(/,/g, "")) || 0;
            let giaMoi = (giaGocNum * (1 - o[key] / 100)).toLocaleString("vi-VN") + " ₫";
            d = "Nhập " + key + " sẽ giảm giá " + o[key] + "% còn " + giaMoi;
        }

        // Tạo khung Flash Sale + Dòng số suất (Chèn ngay trên dòng biến d)
        let flashHtml = "";
        if (ngaySale) {
            flashHtml = `
            <div id="countdown-timer" style="border: 1px solid #e31e24; padding: 15px; margin: 15px 0; border-radius: 8px; text-align: center; background: #fffafb;">
                <p style="font-weight:bold; color:#e31e24; margin-bottom:10px;">🔥 ƯU ĐÃI FLASH SALE KẾT THÚC SAU:</p>
                <div id="countdown" style="display:flex; justify-content:center; margin-bottom:12px;">
                    <div style="background:#333;color:#fff;padding:5px;border-radius:4px;margin:0 3px;min-width:45px;"><span id="days">00</span><br><small style='font-size:9px'>Ngày</small></div>
                    <div style="background:#333;color:#fff;padding:5px;border-radius:4px;margin:0 3px;min-width:45px;"><span id="hours">00</span><br><small style='font-size:9px'>Giờ</small></div>
                    <div style="background:#333;color:#fff;padding:5px;border-radius:4px;margin:0 3px;min-width:45px;"><span id="minutes">00</span><br><small style='font-size:9px'>Phút</small></div>
                    <div style="background:#333;color:#fff;padding:5px;border-radius:4px;margin:0 3px;min-width:45px;"><span id="seconds">00</span><br><small style='font-size:9px'>Giây</small></div>
                </div>
                
                <div id="slots-info" style="font-weight:bold; color:#ff0000; margin-bottom:8px; padding:5px; background:#ffeeee; border-radius:4px;">
                    ⚡ Chỉ còn <span id="slot-count" style="font-size:20px;">${soSuatHienTai}</span> suất đăng ký cuối cùng!
                </div>

                <div class="discount" style="color:#444; font-size:14px; font-weight:500;">${d}</div>
                <div style="font-size:12px; margin-top:5px; color:#666;">(Số lượng có hạn, chương trình sẽ tự kết thúc khi hết suất)</div>
            </div>`;
        }

        // Vẽ lại giao diện trang sản phẩm
        el.innerHTML = `
        <div class="product-container">
            <div class="row">
                <div class="col-sm-6"><img src="${anhChinh}" style="width:100%; border-radius:10px;"></div>
                <div class="col-sm-6">
                    <h1 style="font-size:22px; color:#333;">${title}</h1>
                    <div style="font-size:26px; color:#e31e24; font-weight:bold; margin:10px 0;">${giaBan} ₫</div>
                    <p>Trạng thái: <b style="color:green">${tinhTrangsp}</b></p>
                    ${txtDaBan}
                    <div style="margin:15px 0;">
                        <a href="/p/gio-hang.html" style="background:#e31e24; color:#fff; padding:10px 25px; border-radius:5px; text-decoration:none; font-weight:bold; display:inline-block;">ĐĂNG KÝ NGAY</a>
                    </div>
                    ${flashHtml}
                </div>
            </div>
            <div style="margin-top:25px;">
                <h3 style="border-left:5px solid #e31e24; padding-left:10px; font-size:18px;">NỘI DUNG CHI TIẾT</h3>
                <div style="line-height:1.6;">${moTasp}</div>
            </div>
        </div>`;

        // Kích hoạt bộ đếm
        if (ngaySale) flashsalerun(ngaySale, soSuatHienTai);

    } catch (err) { showDtcError("Lỗi hệ thống: " + err.message); }
}
