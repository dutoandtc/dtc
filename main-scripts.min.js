urlmuangay="/2016/12/mua-ngay.html",urlgiohang="/p/gio-hang.html",urlnewimg="https://blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEg6MLk_01qI7bbmaL7UkmJXOF9WlRIgBaTooLANtJoVHEBzYHA0o9b8bksfItRbBuVnSEUf4fn85AJWclpj53AIbv2i0-S-UEpQtMnWn-DBb6dUFpUC7mUIJyIBVoOjM5ckYTfx4qtGW1KZ/s29/new.gif",newimgwidth=29,newimgheight=19,sumtitle=35,numposts1=6,numposts5=5,newssumtitle=43,newssumposts=370,labelsumtitle=35,labelsumtitle2=20,renewstitle="Tin tức li\xean quan",relatedtitle="Sản phẩm li\xean quan";var purl=location.href,fb_href=purl.substring(0,purl.indexOf(".html")+5),fbcm='<div class="fb-comments" data-href="'+fb_href+'" data-num-posts="12" data-width="100%" colorscheme="light"></div>';function removeHtmlTag(e,t){for(var i=e.split("<"),n=0;n<i.length;n++)-1!=i[n].indexOf(">")&&(i[n]=i[n].substring(i[n].indexOf(">")+1,i[n].length));return(i=i.join("")).substring(0,t-1)}function flashsalerun(e, initialSlots) {
    ...
    let r = setInterval(function() {
        ...
        // Kiểm tra phần tử tồn tại trước khi ghi đè để tránh lỗi crash script
        if (document.getElementById("days")) {
            document.getElementById("days").innerHTML = Math.floor(dist / 864e5);
            document.getElementById("hours").innerHTML = Math.floor(dist % 864e5 / 36e5);
            document.getElementById("minutes").innerHTML = Math.floor(dist % 36e5 / 6e4);
            document.getElementById("seconds").innerHTML = Math.floor(dist % 6e4 / 1e3);
        }

        // Tính số suất
        let duration = 24 * 60 * 60 * 1000; 
        let currentSlots = Math.max(2, Math.ceil(initialSlots * (dist / (dist + duration * 0.5)))); 
        
        let slotEl = document.getElementById("slot-count");
        if (slotEl) slotEl.innerHTML = currentSlots;
    }, 1e3);
}function tinhngay(e){let t=new Date(e),i=new Date-t;return Math.floor(i/864e5)}function displayRandomNumber(){let e=JSON.parse(localStorage.getItem("randomNumberData"))||{},t=e.lastUpdated?new Date(e.lastUpdated):null,i=new Date;if(!t||i-t>=3e5){let n=Math.floor(7*Math.random());return localStorage.setItem("randomNumberData",JSON.stringify({number:n,lastUpdated:i})),n}return e.number}

function chitietsp(e, t, i) {
    window.stopgiamgia = 1;
    var n = "", a = (e = document.getElementById(e)).getElementsByTagName("img");
    1 <= a.length && (n = '<img src="' + a[0].src + '"/>');

    // Hàm phụ để lấy nội dung giữa các thẻ [ ] và loại bỏ mã HTML rác
    var getExtra = function(tagName, defaultVal) {
        var startTag = "[" + tagName + "]";
        var endTag = "[/" + tagName + "]";
        if (e.innerHTML.indexOf(startTag) != -1) {
            var raw = e.innerHTML.substring(e.innerHTML.indexOf(startTag) + startTag.length, e.innerHTML.indexOf(endTag));
            return raw.replace(/<[^>]*>?/gm, '').trim(); // Xóa thẻ HTML nếu Blogger tự chèn vào
        }
        return defaultVal;
    };

    var r = getExtra("giacu", ""),
        s = getExtra("giaban", "0");
    
    giavalue = r.replace(/,/gi, "");
    giagocvalue = s.replace(/,/gi, "");
    var l = parseInt(s.replace(/,/g, ""));

    var c = getExtra("chitiet", "Đang cập nhật nội dung...");
    var tintuc = getExtra("tintuc", "");
    var kythuat = getExtra("kythuat", "Không có thông số kỹ thuật");
    var motangan = getExtra("motangan", "");
    var danhgia = getExtra("danhgia", "Đang cập nhật");
    var daban = getExtra("daban", "");
    var tinhtrang = getExtra("tinhtrang", "Còn hàng");
    
    // Lấy số suất và ngày Flash Sale
    var sosuat = parseInt(getExtra("sosuat", "10")) || 10;
    var flashsale = getExtra("flashsale", "31/12/2026");

    var daban_txt = "";
    if (daban != "") {
        daban_txt = "<p class='inventory'>Đã bán: " + tinhngay(daban) + "</p>";
    }

    var o = {};
    var giamgia = getExtra("giamgia", "");
    if (giamgia != "") {
        giamgia.split(",").forEach(function(item) {
            var parts = item.split("--");
            if (parts.length == 2) o[parts[0].trim()] = parts[1].trim();
        });
    }

    var d = "";
    var n_idx = 0;
    var k = displayRandomNumber();
    for (var u in o) {
        if (o.hasOwnProperty(u)) {
            if (n_idx == k) {
                let p = (l * (1 - o[u] / 100)).toLocaleString("vi-VN", { style: "currency", currency: "VND" });
                d += "Nhập " + u + " sẽ giảm giá " + o[u] + "% còn " + p + "<br>";
            }
            n_idx += 1;
        }
    }
    ...
    flashshow = "";
    if (e.innerHTML.indexOf("[flashsale]") != -1) {
        flashshow = '<div id="countdown-timer" style="border: 2px solid #e31e24; padding: 15px; margin: 15px 0; border-radius: 10px; text-align: center; background: #fffafb;">' +
            '<p style="font-weight:bold; color:#e31e24; margin-bottom:10px; font-size:16px;">🔥 ƯU ĐÃI FLASH SALE KẾT THÚC SAU:</p>' +
            '<div id="countdown" style="margin-bottom:15px;">' +
            '<span class="time-box" style="background:#333; color:#fff; padding:5px 8px; border-radius:4px; margin:0 3px;"><span id="days">00</span> Ngày</span>' +
            '<span class="time-box" style="background:#333; color:#fff; padding:5px 8px; border-radius:4px; margin:0 3px;"><span id="hours">00</span> Giờ</span>' +
            '<span class="time-box" style="background:#333; color:#fff; padding:5px 8px; border-radius:4px; margin:0 3px;"><span id="minutes">00</span> Phút</span>' +
            '<span class="time-box" style="background:#333; color:#fff; padding:5px 8px; border-radius:4px; margin:0 3px;"><span id="seconds">00</span> Giây</span>' +
            '</div>' +
            '<div id="slot-container" style="font-weight:bold; color:#ff0000; padding: 8px; background: #ffeeee; border-radius: 5px; display: inline-block; width: 100%; box-sizing: border-box;">' +
            '⚡ Chỉ còn <span id="slot-count" style="font-size:20px;">' + sosuat + '</span> suất đăng ký cuối cùng!' +
            '</div>' +
            '<div class="discount" style="margin-top:10px; color:#555;">' + d + '</div>' +
            '<div class="promo-code" style="margin-top:5px; font-weight:bold;">Mã ưu đãi: ' + (Object.keys(o)[0] || "DTC2026") + '</div>' +
            '</div>';
    }

    // Thiết lập lại toàn bộ HTML cho trang chi tiết
    e.innerHTML = -1 != e.innerHTML.indexOf("[tintuc]") ? "<h1>" + t + '</h1><div class="content-page"><div class="clearfix"><p class="time pull-left">Đăng bởi:QTV</p><div class="socials pull-right"><div class="fb-like" data-layout="button" data-action="like" data-size="small" data-show-faces="true" data-share="true"></div></div></div><div class="body-content">' + tintuc + "</div></div>" : '<div id="wrapper-detail" class="simpleCart_shelfItem"><div class="row"><a id="thumb" class="item_thumb" style="display:none"><img src="' + a[0].src + '"></a><div class="col-sm-6 col-xs-12"><div id="surround"><div class="img-product"><img class=" product-image-feature hidden-xs" src="' + a[0].src + '" alt="' + t + '"></div></div></div><div class="col-sm-6 col-xs-12 fix-mobile"><div class="product-title"><h1 class="item_name">' + t + '</h1></div><div class="product-price" id="price-preview"><span class="item_price">' + s + '₫</span></div><p class="inventory">Tình trạng: <strong>' + tinhtrang + "</strong></p>" + daban_txt + '<div id="add-item-form" class="variants clearfix"><div class="select-wrapper"><label>Số lượng : </label><input id="quantity" type="number" name="quantity" min="1" value="1" class="item-quantity item_quantity" /></div><div class="actions-btn"><a href="/p/gio-hang.html"><button id="add-to-cart" class=" btn-add-cart btn-fix item_add"> Mua ngay </button></a></div></div> <div class="discount-code"><label>Mã giảm giá:</label><input type="text" id="discount-input"><button onclick="applyDiscount(' + l + ')">Áp dụng</button></div>' + flashshow + '</div><div class="col-xs-12"><div class="product-comment"><h4><span>Mô tả</span></h4><div id="mota"><div class="product-description-wrapper">' + c + "</div></div></div></div></div></div>";

    // Khởi chạy đồng hồ
    flashsalerun(flashsale, sosuat);
}

function relatedproduct(e){let t=document.getElementById("like");if(!t){console.error("Kh\xf4ng t\xecm thấy v\xf9ng chứa cho sản phẩm li\xean quan: #like");return}let i="",n=e.feed.entry||[],a=Math.min(relatednumposts,n.length),r=n.length>0&&n[0].content.$t.includes("[tintuc]"),s=r?renewstitle:relatedtitle;i+=`<div class="title-line"><h4><span>${s}</span></h4></div><ul class="row clearfix content-product-list">`;for(let l=0;l<a;l++){let c=n[l];if(!c)continue;let o=c.title.$t,d="";for(let u=0;u<c.link.length;u++)if("alternate"===c.link[u].rel){d=c.link[u].href;break}let p="content"in c?c.content.$t:"summary"in c?c.summary.$t:"",h=e=>{let t=`[${e}]`,i=`[/${e}]`;return p.includes(t)&&p.includes(i)?p.substring(p.indexOf(t)+t.length,p.indexOf(i)):""},m="",g=h("giamgia"),f=0;if(g){let y=g.split(",")[0].trim();if(y.includes("--")){let v=y.split("--")[1].trim();v&&!isNaN(v)&&(m+=`<span class="product-status discount">Giảm ${f=parseFloat(v)}%</span>`)}}let _=h("tinhtrang");_&&(m+=`<span class="product-status tinhtrang">${_}</span>`);let b=h("daban");b&&(m+=`<span class="product-status daban">Đ\xe3 b\xe1n: ${tinhngay(b)}</span>`);let C="",x=h("giaban"),T=x?parseInt(x.replace(/,/g,""),10):0,L=e=>isNaN(e)?"Li\xean hệ":e.toLocaleString("vi-VN")+"₫";if(f>0&&T>0){let H=Math.round(T*(1-f/100));C=`<span class="new-price item_price">${L(H)}</span>
        ...
        <div class="col-xs-12 col-sm-12 col-md-12 simpleCart_shelfItem">
          <div class="product-loop">
            <a href="${i}" class="product-image-container">
              <img class="item_thumb lozad" data-src="${v}" src="data:image/gif;base64,R0lGODlhAQABAIAAAP///wAAACH5BAEAAAAALAAAAAABAAEAAAICRAEAOw==" alt="${t}">
              ${s}
            </a>
            <div class="product-info clearfix">
            ...
            <div class="col-xs-12 col-sm-12">
                <div class="blog-loop clearfix">
                    <div class="blog-img pull-left col-12">
                        <a href="${i}">
                            <img class="lozad" data-src="${r}" src="data:image/gif;base64,R0lGODlhAQABAIAAAP///wAAACH5BAEAAAAALAAAAAABAAEAAAICRAEAOw==" alt="${t}">
                        </a>
                    </div>
            ...
            <li>
                <div class="col-xs-12 col-sm-12">
                    <div class="blog-loop clearfix">
                        <div class="blog-img pull-left col-12">
                            <a href="${l}">
                                <img class="lozad" data-src="${m}" src="data:image/gif;base64,R0lGODlhAQABAIAAAP///wAAACH5BAEAAAAALAAAAAABAAEAAAICRAEAOw==" alt="${s}">
                            </a>
                        </div>
                        <div class="blog-info pull-right col-12">
                            <h3><a href="${l}">${s}</a></h3>
