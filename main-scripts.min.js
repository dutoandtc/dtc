...
function chitietsp(e, t, i) {
    ...
    var productEl = document.getElementById(e);
    if (!productEl) return; // Kiểm tra an toàn nếu không tìm thấy phần tử

    var n = "", a = productEl.getElementsByTagName("img");
    if (a.length >= 1) n = '<img src="' + a[0].src + '"/>';

    // Hàm phụ lấy nội dung thẻ [ ] thông minh, loại bỏ mã rác của Blogger
    var getExtra = function(tagName, defaultVal) {
        var html = productEl.innerHTML;
        // Tìm kiếm không phân biệt hoa thường
        var startTag = "[" + tagName.toLowerCase() + "]";
        var endTag = "[/" + tagName.toLowerCase() + "]";
        var lowerHtml = html.toLowerCase();
        
        var startIdx = lowerHtml.indexOf(startTag);
        var endIdx = lowerHtml.indexOf(endTag);
        
        if (startIdx != -1 && endIdx != -1) {
            var raw = html.substring(startIdx + startTag.length, endIdx);
    ...
    var r = getExtra("giacu", ""),
        s = getExtra("giaban", "0");
    var giavalue = r.replace(/,/gi, ""), 
        giagocvalue = s.replace(/,/gi, "");
    var l = parseInt(s.replace(/,/g, "")) || 0;
    ...
    var daban_txt = daban != "" ? "<p class='inventory'>Đã bán: " + tinhngay(daban) + "</p>" : "";
    ...
    if (productEl.innerHTML.indexOf("[flashsale]") != -1) {
        flashshow = '<div id="countdown-timer" style="border: 2px solid #e31e24; padding: 15px; margin: 15px 0; border-radius: 10px; text-align: center; background: #fffafb;">' +
            '<p style="font-weight:bold; color:#e31e24; margin-bottom:10px;">🔥 ƯU ĐÃI FLASH SALE KẾT THÚC SAU:</p>' +
            '<div id="countdown" style="margin-bottom:15px; display:flex; justify-content:center;">' +
            '<span class="time-box" style="background:#333; color:#fff; padding:5px; border-radius:4px; margin:0 3px; min-width:50px;"><span id="days">00</span><br><small>Ngày</small></span>' +
            '<span class="time-box" style="background:#333; color:#fff; padding:5px; border-radius:4px; margin:0 3px; min-width:50px;"><span id="hours">00</span><br><small>Giờ</small></span>' +
            '<span class="time-box" style="background:#333; color:#fff; padding:5px; border-radius:4px; margin:0 3px; min-width:50px;"><span id="minutes">00</span><br><small>Phút</small></span>' +
            '<span class="time-box" style="background:#333; color:#fff; padding:5px; border-radius:4px; margin:0 3px; min-width:50px;"><span id="seconds">00</span><br><small>Giây</small></span>' +
            '</div>' +
            '<div id="slot-container" style="font-weight:bold; color:#ff0000; padding: 8px; background: #ffeeee; border-radius: 5px; display: block; margin-top:10px;">' +
            '⚡ Chỉ còn <span id="slot-count" style="font-size:22px;">' + sosuat + '</span> suất đăng ký cuối cùng!' +
            '</div>' +
            '<div class="discount" style="margin-top:10px; color:#555;">' + d + '</div>' +
            '<div class="promo-code" style="margin-top:5px; font-weight:bold;">Mã ưu đãi: ' + (keys[0] || "DTC2026") + '</div>' +
            '</div>';
    }

    // Gán nội dung vào giao diện
    if (productEl.innerHTML.indexOf("[tintuc]") != -1) {
        productEl.innerHTML = "<h1>" + t + '</h1><div class="content-page"><div class="body-content">' + tintuc + "</div></div>";
    } else {
        productEl.innerHTML = '<div id="wrapper-detail" class="simpleCart_shelfItem"><div class="row">' +
            '<div class="col-sm-6 col-xs-12"><div id="surround"><div class="img-product"><img class="product-image-feature" src="' + a[0].src + '" alt="' + t + '"></div></div></div>' +
            '<div class="col-sm-6 col-xs-12 fix-mobile"><div class="product-title"><h1 class="item_name">' + t + '</h1></div>' +
            '<div class="product-price"><span class="item_price">' + s + '₫</span></div>' +
            '<p class="inventory">Tình trạng: <strong>' + tinhtrang + '</strong></p>' + daban_txt +
            '<div id="add-item-form" class="variants clearfix"><div class="select-wrapper"><label>Số lượng : </label><input id="quantity" type="number" name="quantity" min="1" value="1" class="item-quantity item_quantity" /></div><div class="actions-btn"><a href="/p/gio-hang.html"><button id="add-to-cart" class="btn-add-cart btn-fix item_add"> Mua ngay </button></a></div></div>' +
            '<div class="discount-code"><label>Mã giảm giá:</label><input type="text" id="discount-input"><button onclick="applyDiscount(' + l + ')">Áp dụng</button></div>' + 
            flashshow + '</div>' +
            '<div class="col-xs-12"><div class="product-comment"><h4><span>Mô tả</span></h4><div id="mota"><div class="product-description-wrapper">' + c + '</div></div></div></div>' +
            '</div></div>';
    }
