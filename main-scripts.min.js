...
// Hàm hiển thị thông báo lỗi trực tiếp trên màn hình
function showDtcError(msg) {
    let errorDiv = document.getElementById("dtc-debug-log");
    if (!errorDiv) {
        errorDiv = document.createElement("div");
        errorDiv.id = "dtc-debug-log";
        errorDiv.style = "position:fixed; top:10px; left:10px; background:rgba(255,0,0,0.9); color:white; padding:15px; border-radius:5px; z-index:9999; font-family:monospace; font-size:12px; max-width:300px; box-shadow:0 0 10px rgba(0,0,0,0.5);";
        document.body.appendChild(errorDiv);
    }
    errorDiv.innerHTML += "<div>• " + msg + "</div>";
    console.error("DTC Debug: " + msg);
}
function flashsalerun(e, initialSlots) {
    try {
        if (!e || e.trim() === "") {
            showDtcError("Lỗi: Thẻ [flashsale] đang để trống!");
            return;
        }

        let parts = e.split("/");
        if (parts.length !== 3) {
            showDtcError("Lỗi định dạng ngày: '" + e + "'. Phải là Ngày/Tháng/Năm (VD: 25/12/2026)");
            return;
        }
        ...
        if (isNaN(endTime)) {
            showDtcError("Lỗi: Ngày tháng không hợp lệ (" + e + ")");
            return;
        }
        ...
        showDtcError("Hàm flashsalerun gặp lỗi: " + err.message);
    }
}
...
function chitietsp(e, t, i) {
    try {
        ...
        if (!productEl) {
            showDtcError("Không tìm thấy vùng chứa sản phẩm với ID: " + e);
            return;
        }

        // Hàm lấy dữ liệu an toàn và báo lỗi nếu thiếu
        var getExtra = function(tagName, defaultVal) {
            ...
            var startIdx = html.toLowerCase().indexOf(startTag);
            var endIdx = html.toLowerCase().indexOf(endTag);
            
            if (startIdx != -1 && endIdx != -1) {
                return html.substring(startIdx + startTag.length, endIdx).replace(/<[^>]*>?/gm, '').trim();
            }
            if (tagName === "flashsale" || tagName === "sosuat") {
                showDtcError("Cảnh báo: Thiếu thẻ [" + tagName + "] trong bài viết.");
            }
            return defaultVal;
        };
        ...
        var r = getExtra("giacu", ""),
            s = getExtra("giaban", "0"),
            l = parseInt(s.replace(/,/g, "")) || 0,
            sosuat = parseInt(getExtra("sosuat", "10")) || 10,
            flashsale = getExtra("flashsale", "31/12/2026"),
            tinhtrang = getExtra("tinhtrang", "Còn hàng"),
            daban = getExtra("daban", ""),
            c = getExtra("chitiet", "Nội dung đang cập nhật...");

        var imgs = productEl.getElementsByTagName("img");
        var firstImg = imgs.length >= 1 ? imgs[0].src : "";

        var daban_txt = daban != "" ? "<p class='inventory'>Đã bán: " + tinhngay(daban) + "</p>" : "";

        // Xử lý mã giảm giá
        var o = {}, d = "", giamgia = getExtra("giamgia", "");
        if (giamgia != "") {
            giamgia.split(",").forEach(item => {
                var p = item.split("--");
                if (p.length == 2) o[p[0].trim()] = p[1].trim();
            });
            var keys = Object.keys(o);
            var u = keys[0]; // Lấy mã đầu tiên làm ví dụ
            let priceDis = (l * (1 - o[u] / 100)).toLocaleString("vi-VN") + "₫";
            d = "Mã " + u + " giảm " + o[u] + "% còn " + priceDis;
        }

        var flashshow = "";
        if (productEl.innerHTML.indexOf("[flashsale]") != -1) {
            flashshow = `
            <div id="countdown-timer" style="border: 2px solid #e31e24; padding: 15px; margin: 15px 0; border-radius: 10px; text-align: center; background: #fffafb;">
                <p style="font-weight:bold; color:#e31e24; margin-bottom:10px;">🔥 ƯU ĐÃI KẾT THÚC SAU:</p>
                <div id="countdown" style="display:flex; justify-content:center; margin-bottom:10px;">
                    <span style="background:#333; color:#fff; padding:5px; border-radius:4px; margin:0 3px; min-width:45px;"><span id="days">00</span><br><small>Ngày</small></span>
                    <span style="background:#333; color:#fff; padding:5px; border-radius:4px; margin:0 3px; min-width:45px;"><span id="hours">00</span><br><small>Giờ</small></span>
                    <span style="background:#333; color:#fff; padding:5px; border-radius:4px; margin:0 3px; min-width:45px;"><span id="minutes">00</span><br><small>Phút</small></span>
                    <span style="background:#333; color:#fff; padding:5px; border-radius:4px; margin:0 3px; min-width:45px;"><span id="seconds">00</span><br><small>Giây</small></span>
                </div>
                <div id="slot-container" style="font-weight:bold; color:#ff0000; padding: 10px; background: #ffeeee; border-radius: 5px; display: block;">
                    ⚡ Chỉ còn <span id="slot-count" style="font-size:24px;">${sosuat}</span> suất cuối cùng!
                </div>
                <div style="margin-top:10px; font-size:13px; color:#666;">${d}</div>
            </div>`;
        }

        // Render HTML
        productEl.innerHTML = `
        <div id="wrapper-detail" class="simpleCart_shelfItem">
            <div class="row">
                <div class="col-sm-6"><img src="${firstImg}" style="width:100%; border-radius:8px;"></div>
                <div class="col-sm-6 fix-mobile">
                    <h1 class="item_name">${t}</h1>
                    <div class="product-price"><span class="item_price" style="font-size:28px; color:#e31e24; font-weight:bold;">${s}₫</span></div>
                    <p>Tình trạng: <strong>${tinhtrang}</strong></p>
                    ${daban_txt}
                    <div class="actions-btn" style="margin:20px 0;">
                        <a href="/p/gio-hang.html" style="background:#e31e24; color:#fff; padding:12px 25px; border-radius:5px; text-decoration:none; font-weight:bold; display:inline-block;">MUA NGAY</a>
                    </div>
                    ${flashshow}
                </div>
                <div class="col-xs-12" style="margin-top:30px;">
                    <h4 style="border-bottom:2px solid #eee; padding-bottom:10px;">MÔ TẢ CHI TIẾT</h4>
        ...
        showDtcError("Lỗi hệ thống (chitietsp): " + globalErr.message);
    }
}
