urlmuangay="/2016/12/mua-ngay.html",urlgiohang="/p/gio-hang.html",urlnewimg="https://blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEg6MLk_01qI7bbmaL7UkmJXOF9WlRIgBaTooLANtJoVHEBzYHA0o9b8bksfItRbBuVnSEUf4fn85AJWclpj53AIbv2i0-S-UEpQtMnWn-DBb6dUFpUC7mUIJyIBVoOjM5ckYTfx4qtGW1KZ/s29/new.gif",newimgwidth=29,newimgheight=19,sumtitle=35,numposts1=6,numposts5=5,newssumtitle=43,newssumposts=370,labelsumtitle=35,labelsumtitle2=20,renewstitle="Tin tức li\xean quan",relatedtitle="Sản phẩm li\xean quan";var purl=location.href,fb_href=purl.substring(0,purl.indexOf(".html")+5),fbcm='<div class="fb-comments" data-href="'+fb_href+'" data-num-posts="12" data-width="100%" colorscheme="light"></div>';function removeHtmlTag(e,t){for(var i=e.split("<"),n=0;n<i.length;n++)-1!=i[n].indexOf(">")&&(i[n]=i[n].substring(i[n].indexOf(">")+1,i[n].length));return(i=i.join("")).substring(0,t-1)}function flashsalerun(e){let[t,i,n]=e.split("/").map(Number),a=new Date(n,i-1,t).getTime(),r=setInterval(function(){let e=(new Date).getTime(),t=a-e;document.getElementById("days").innerHTML=Math.floor(t/864e5),document.getElementById("hours").innerHTML=Math.floor(t%864e5/36e5),document.getElementById("minutes").innerHTML=Math.floor(t%36e5/6e4),document.getElementById("seconds").innerHTML=Math.floor(t%6e4/1e3),console.log(t),t<0&&(clearInterval(r),document.getElementById("countdown").innerHTML="Flash Sale Đã Kết Thúc!",document.getElementsByClassName("discount")[0].style.display="none",document.getElementsByClassName("promo-code")[0].style.display="none",window.stopgiamgia=0)
},1e3)}function tinhngay(e){let t=new Date(e),i=new Date-t;return Math.floor(i/864e5)}function displayRandomNumber(){try{let e=JSON.parse(localStorage.getItem("randomNumberData"))||{},t=e.lastUpdated?new Date(e.lastUpdated):null,i=new Date;if(!t||i-t>=3e5){let n=Math.floor(7*Math.random());return localStorage.setItem("randomNumberData",JSON.stringify({number:n,lastUpdated:i})),n}return e.number||0}catch(err){return Math.floor(7*Math.random())}}function chitietsp(e, t, i) {
    window.stopgiamgia = 1;
    var element = document.getElementById(e);
    if (!element) return; // Chống crash nếu phần tử chưa kịp tải trên mobile
    
    var a = element.getElementsByTagName("img");
    var htmlContent = element.innerHTML;
    var defaultImg = "data:image/gif;base64,R0lGODlhAQABAIAAAP///wAAACH5BAEAAAAALAAAAAABAAEAAAICRAEAOw==";

    // HÀM BỌC THÉP: Trích xuất shortcode, tự động diệt thẻ HTML rác do Blogger chèn vào
    function getVal(code, isNum, keepHtml) {
        var start = "[" + code + "]";
        var end = "[/" + code + "]";
        var idx1 = htmlContent.indexOf(start);
        var idx2 = htmlContent.indexOf(end);
        if (idx1 !== -1 && idx2 !== -1 && idx2 > idx1) {
            var raw = htmlContent.substring(idx1 + start.length, idx2);
            if (!keepHtml) {
                // Quét sạch thẻ <br>, <div>, <span>, &nbsp;
                raw = raw.replace(/(<([^>]+)>)/ig, "").replace(/&nbsp;/ig, "").trim();
            } else {
                raw = raw.trim();
            }
            if (isNum) {
                // Chỉ giữ lại đúng chữ số, chống sập hàm Number
    ...
    // --- LẤY DỮ LIỆU AN TOÀN ---
    var r_val = getVal("giacu", true, false);
    var l = getVal("giaban", true, false);
    var s = l > 0 ? l.toLocaleString("vi-VN") : "Liên hệ";
    
    var c = getVal("chitiet", false, true);
    var tintuc = getVal("tintuc", false, true);
    var daban = getVal("daban", false, false);
    var daban_txt = daban !== "" ? "<p class='inventory'>Đã bán: " + tinhngay(daban) + "</p>" : "";
    var tinhtrang = getVal("tinhtrang", false, true) || "Đang cập nhật";
    var flashsale = getVal("flashsale", false, false) || "14/11/2024";
    var km = getVal("km", false, true) || "Đang cập nhật ....";
    var giamgia = getVal("giamgia", false, false);

    // Xử lý mã giảm giá
    var o = {};
    if (giamgia !== "") {
        giamgia.split(",").forEach(function (e) {
            var arr = e.split("--");
            if (arr.length >= 2) {
                o[arr[0].trim()] = arr[1].trim();
            }
        });
    }
    ...
    var d = "";
    var count = 0;
    var k = displayRandomNumber(); // Dùng hàm đã fix Try-Catch lần trước
    for (var u in o) {
        if (o.hasOwnProperty(u)) {
            if (count == k) {
                var giam = parseFloat(o[u]);
                var priceAfter = l * (1 - giam / 100);
                var p_str = "";
                try {
                    p_str = priceAfter.toLocaleString("vi-VN", { style: "currency", currency: "VND" });
                } catch (err) {
                    p_str = priceAfter.toLocaleString("vi-VN") + " đ";
                }
                d += "Nhập " + u + " sẽ giảm giá " + o[u] + "% còn " + p_str + "<br>";
            }
            count += 1;
        }
    }
    ...
    var flashshow = "";
    if (giamgia !== "") {
        flashshow = '<div id="countdown-timer"> <p>Flash Sale Kết Thúc Sau:</p> <div id="countdown"> <span class="time-box"><span id="days">00</span><span class="label">Ngày</span></span> <span class="time-box"><span id="hours">00</span><span class="label">Giờ</span></span> <span class="time-box"><span id="minutes">00</span><span class="label">Phút</span></span> <span class="time-box"><span id="seconds">00</span><span class="label">Giây</span></span> </div> <div class="discount">' + d + '</div><div class="promo-code">Mã ưu đãi: ' + (Object.keys(o)[0] || "TIETKIEM") + "</div></div>";
    }

    window.applyDiscount = function (e) {
        if (window.stopgiamgia == 1) {
            var t = document.getElementById("discount-input").value.trim();
            if (o.hasOwnProperty(t)) {
                var i = parseFloat(o[t]);
                var priceAfter = e * (1 - i / 100);
                var n = "";
                try { n = priceAfter.toLocaleString("vi-VN", { style: "currency", currency: "VND" }).replace(/\./g, ","); } 
                catch (err) { n = priceAfter.toLocaleString("vi-VN") + " đ"; }
                
                document.querySelector(".item_price").innerText = n;
                var nameEl = document.querySelector(".item_name");
                var originalName = nameEl.getAttribute("data-original-name") || nameEl.innerText;
                nameEl.setAttribute("data-original-name", originalName);
                
                var giaGocStr = "";
                try { giaGocStr = e.toLocaleString("vi-VN", { style: "currency", currency: "VND" }).replace(/\./g, ","); } 
                catch(err) { giaGocStr = e.toLocaleString("vi-VN") + " đ"; }
                
                nameEl.innerText = originalName + " - Mã giảm giá " + t + " (" + i + "%, giá gốc " + giaGocStr + ")";
            }
    ...
    // Xử lý bộ sưu tập ảnh an toàn (Chống sập nếu không có ảnh)
    var m = "", g = "";
    for (var idx = 0; idx < Math.min(4, a.length); idx++) {
        var src = a[idx].src;
        m += '<div class="item product-thumb"><a href="javascript:void(0)" data-image="' + src + '" data-zoom-image="' + src + '"><img alt="' + t + '" data-image="' + src + '" src="' + src + '" ></a></div>';
        g += '<li class="item product-thumb"><a href="javascript:void(0)" data-image="' + src + '" data-zoom-image="' + src + '"><img alt="' + t + '" data-image="' + src + '" src="' + src + '" ></a></li>';
    }
    var mainImg = a.length > 0 ? a[0].src : defaultImg;

    // HIỂN THỊ GIAO DIỆN
    if (htmlContent.indexOf("[tintuc]") !== -1) {
        element.innerHTML = "<h1>" + t + '</h1><div class="content-page"><div class="clearfix"><p class="time pull-left">Đăng bởi: QTV</p><div class="socials pull-right"><div class="fb-like" data-layout="button" data-action="like" data-size="small" data-show-faces="true" data-share="true"></div></div></div><div class="body-content">' + tintuc + "</div></div>";
    } else {
        element.innerHTML = '<div id="wrapper-detail" class="simpleCart_shelfItem"><div class="row"><a id="thumb" class="item_thumb" style="display:none"><img src="' + mainImg + '"></a><div class="col-sm-6 col-xs-12"><div id="surround"><div class="img-product"><img class=" product-image-feature hidden-xs" src="' + mainImg + '" alt="' + t + '"></div><div class="thumb-mt20"><div id="sliderproduct" class="clearfix hidden-xs">' + m + '</div><div id="sliderproduct-mobile" class="visible-xs"><div class="slider-container "><ul class="slides clearfix">' + g + '</ul></div></div></div></div></div><div class="col-sm-6 col-xs-12 fix-mobile"><div class="product-title"><h1 class="item_name">' + t + '</h1></div><div class="product-price" id="price-preview"><span class="item_price">' + s + '₫</span></div><p class="inventory">Tình trạng: <strong>' + tinhtrang + "</strong></p>" + daban_txt + '<div id="add-item-form" action="/cart/add" method="post" class="variants clearfix"><div class="select-wrapper"><label>Số lượng : </label><input id="quantity" type="number" name="quantity" min="1" value="1" class="item-quantity item_quantity" /></div><div class="actions-btn"><a href="' + urlgiohang + '"><button id="add-to-cart" class=" btn-add-cart btn-fix item_add" name="add"> Mua ngay </button></a></div></div> <div class="discount-code"><label for="discount-input">Mã giảm giá:</label><input type="text" id="discount-input" placeholder="Nhập mã giảm giá"><button onclick="applyDiscount(' + l + ')">Áp dụng</button></div><p id="discount-message" style="color: red; display: none;">Mã giảm giá không hợp lệ!</p>' + flashshow + '<div class="bonus"><p class="title">Khuyến mãi</p><ul class="bonus-list">' + km + '</ul></div></div><div class="col-xs-12"><div class="product-comment"><h4><span>Mô tả</span></h4><div id="mota"><div class="product-description-wrapper">' + c + "</div></div></div></div></div></div>";
        flashsalerun(flashsale);
    }
