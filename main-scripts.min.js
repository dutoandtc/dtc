...
function flashsalerun(e, initialSlots) {
    try {
        if (!e) throw new Error("Thiếu ngày Flash Sale");
        let parts = e.split("/");
        if (parts.length !== 3) throw new Error("Định dạng ngày phải là Ngày/Tháng/Năm (VD: 20/12/2026)");

        let [day, month, year] = parts.map(Number);
        let endTime = new Date(year, month - 1, day).getTime();

        if (isNaN(endTime)) throw new Error("Ngày tháng không hợp lệ: " + e);
        ...
        let r = setInterval(function() {
            ...
            if (dist < 0) {
                ...
                console.log("DTC Debug: Flash Sale đã hết hạn, ẩn khung.");
                return;
            }

            // Cập nhật đồng hồ kèm kiểm tra phần tử
            const setVal = (id, val) => {
                let el = document.getElementById(id);
                if (el) el.innerHTML = val;
                else console.warn("DTC Debug: Không tìm thấy thẻ ID: " + id);
            };

            setVal("days", Math.floor(dist / 864e5));
            setVal("hours", Math.floor(dist % 864e5 / 36e5));
            setVal("minutes", Math.floor(dist % 36e5 / 6e4));
            setVal("seconds", Math.floor(dist % 6e4 / 1e3));

            // Tính số suất
            let duration = 24 * 60 * 60 * 1000; 
            let currentSlots = Math.max(2, Math.ceil(initialSlots * (dist / (dist + duration * 0.5)))); 
            setVal("slot-count", currentSlots);

        }, 1e3);
    } catch (err) {
        console.error("DTC Error (flashsalerun): " + err.message);
    }
}
...
function chitietsp(e, t, i) {
    try {
        ...
        if (!productEl) {
            console.error("DTC Error: Không tìm thấy ID sản phẩm: " + e);
            return;
        }

        console.log("DTC Debug: Bắt đầu bóc tách sản phẩm: " + t);

        var imgs = productEl.getElementsByTagName("img");
        var firstImg = imgs.length >= 1 ? imgs[0].src : "https://via.placeholder.com/400";
        ...
        var getExtra = function(tagName, defaultVal) {
            ...
            if (startIdx != -1 && endIdx != -1) {
                var res = html.substring(startIdx + startTag.length, endIdx).replace(/<[^>]*>?/gm, '').trim();
                console.log("DTC Debug: Tìm thấy thẻ [" + tagName + "] = " + res);
                return res;
            }
            console.warn("DTC Debug: Thiếu thẻ [" + tagName + "], dùng giá trị mặc định: " + defaultVal);
            return defaultVal;
        };
        ...
        if (giamgia != "") {
            ...
            var u = keys[displayRandomNumber() % keys.length];
            let priceDis = (l * (1 - o[u] / 100)).toLocaleString("vi-VN") + "₫";
            d = "Nhập " + u + " giảm " + o[u] + "% còn " + priceDis;
        }

        // Tạo khung Flash Sale
        var flashshow = "";
        if (productEl.innerHTML.indexOf("[flashsale]") != -1) {
            ...
            <div id="countdown-timer" style="border: 2px solid #e31e24; padding: 15px; margin: 15px 0; border-radius: 10px; text-align: center; background: #fffafb;">
                ...
                <div id="countdown" style="display:flex; justify-content:center; margin-bottom:10px;">
                    <span class="time-box" style="background:#333; color:#fff; padding:5px 10px; border-radius:4px; margin:0 3px;"><span id="days">00</span><br><small>Ngày</small></span>
                    <span class="time-box" style="background:#333; color:#fff; padding:5px 10px; border-radius:4px; margin:0 3px;"><span id="hours">00</span><br><small>Giờ</small></span>
                    <span class="time-box" style="background:#333; color:#fff; padding:5px 10px; border-radius:4px; margin:0 3px;"><span id="minutes">00</span><br><small>Phút</small></span>
                    <span class="time-box" style="background:#333; color:#fff; padding:5px 10px; border-radius:4px; margin:0 3px;"><span id="seconds">00</span><br><small>Giây</small></span>
                </div>
            ...
            <div class="row">
                ...
                <div class="col-sm-6">
                    ...
                    <div class="actions-btn" style="margin:20px 0;">
                        <a href="/p/gio-hang.html" class="btn-add-cart" style="background:#e31e24; color:#fff; padding:12px 25px; border-radius:5px; text-decoration:none; font-weight:bold; display:inline-block;">MUA NGAY</a>
                    </div>
                    ${flashshow}
                </div>
                <div class="col-xs-12" style="margin-top:30px;">
                    <h4 style="border-bottom:2px solid #eee; padding-bottom:10px;">MÔ TẢ SẢN PHẨM</h4>
                    <div class="description">${c}</div>
                </div>
            </div>
        </div>`;

        // Chạy đồng hồ sau khi HTML đã được nạp
        flashsalerun(flashsale, sosuat);

    } catch (globalErr) {
        console.error("DTC Critical Error: " + globalErr.message);
    }
}
